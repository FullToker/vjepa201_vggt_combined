...
# Run with ./vendor_matterport_download.py (or python3 vendor_matterport_download.py on Windows)
# -*- coding: utf-8 -*-
#
# Python 3 port of the official TUM download_mp.py (vendored verbatim copy
# kept at ckpts/download_mp.py -- see that file for the original). Changes
# from the original, everything else left as-is:
#   - urllib.urlopen/urlretrieve (py2) -> urllib.request equivalents
...
def main():
    parser = argparse.ArgumentParser(description=
        ...
        Downloads MP public data release.
        Example invocation:
          python3 vendor_matterport_download.py -o base_dir --id ALL --type object_segmentations --task_data semantic_voxel_label_data semantic_voxel_label_models
        ...
        The --yes argument skips the interactive confirmation prompts (required for a non-interactive/sbatch run) --
        only pass it once you've actually read the MP Terms of Use at the URL this script prints.
        ''',
    ...
    parser.add_argument('--yes', '-y', action='store_true', help='skip interactive confirmation prompts (required for non-interactive/sbatch runs)')
